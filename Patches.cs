...
namespace Obeliskial_Essentials
{
    ...
    internal class Patches
    {
        ...
        public static Dictionary<string, CardRealtimeData> medsCardsSourceGlobal = new();
        public static bool medsExportNode = false;

        public static void ExportAllData()
        {
            DataTextConvert.ExportAllData();
        }
        ...
        public static void CreateGameContentRoutinePostfix(ref IEnumerator __result)
        {
        ...
        private static void CreateGameContentPostfix()
        {
            ...
            medsCardsSourceGlobal = medsCardsSource;
            foreach (string cardID in medsCardsSource.Keys)
            {
                CardRealtimeData card = medsCardsSource[cardID];
            ...
            LogDebug("Finished setting MedsCards");
            ...
            UniverseLib.Universe.Init(1f, ProfileEditor.Init, Essentials.LogHandler, new()
            {
        ...
        private static void ReorderSubclasses()
        {
        ...
        private static IEnumerator medsHeroSelectionStartCoContinue()
        {
            ...
            if (GameManager.Instance.IsMultiplayer())
            {
                ...
                else
                {
                    ...
                    if (roomName != "")
                    {
                        SaveManager.Instance.SaveIntoPrefsString("coopRoomId", roomName);
            ...
            if (GameManager.Instance.IsGameAdventure())
            {
                ...
                if (GameManager.Instance.IsMultiplayer())
                {
                    if (NetworkManager.Instance.IsMaster())
                    {
                        ...
                        else if (SaveManager.Instance.PrefsHasKey("madnessLevelCoop") && SaveManager.Instance.PrefsHasKey("madnessCorruptorsCoop"))
                        {
                            int num6 = SaveManager.Instance.LoadPrefsInt("madnessLevelCoop");
                            string text3 = SaveManager.Instance.LoadPrefsString("madnessCorruptorsCoop");
                ...
                else if (SaveManager.Instance.PrefsHasKey("madnessLevel") && SaveManager.Instance.PrefsHasKey("madnessCorruptors"))
                {
                    // LogDebug("Found madness prefs");
                    int num8 = SaveManager.Instance.LoadPrefsInt("madnessLevel");
                    string text4 = SaveManager.Instance.LoadPrefsString("madnessCorruptors");
            ...
            else if (GameManager.Instance.IsSingularity())
            {
                ...
                if (GameManager.Instance.IsMultiplayer())
                {
                    if (NetworkManager.Instance.IsMaster())
                    {
                        ...
                        else if (SaveManager.Instance.PrefsHasKey("singularityMadnessCoop"))
                        {
                            int num9 = SaveManager.Instance.LoadPrefsInt("singularityMadnessCoop");
                ...
                else if (SaveManager.Instance.PrefsHasKey("singularityMadness"))
                {
                    // LogDebug("Found singularity madness prefs");
                    int num10 = SaveManager.Instance.LoadPrefsInt("singularityMadness");
            ...
            else if (!GameManager.Instance.IsWeeklyChallenge())
            {
                ...
                if (GameManager.Instance.IsMultiplayer())
                {
                    if (NetworkManager.Instance.IsMaster())
                    {
                        ...
                        else if (SaveManager.Instance.PrefsHasKey("obeliskMadnessCoop"))
                        {
                            int num11 = SaveManager.Instance.LoadPrefsInt("obeliskMadnessCoop");
                ...
                else if (SaveManager.Instance.PrefsHasKey("obeliskMadness"))
                {
                    int num12 = SaveManager.Instance.LoadPrefsInt("obeliskMadness");
            ...
            if (GameManager.Instance.IsMultiplayer())
            {
                ...
                if (NetworkManager.Instance.IsMaster() && GameManager.Instance.IsLoadingGame())
                {
                    ...
                    for (int m = 0; m < 4; m++)
                    {
                        Hero hero = AtOManager.Instance.team.GetHero(m);
            ...
            if (!GameManager.Instance.IsWeeklyChallenge())
            {
                ...
                if (!GameManager.Instance.IsMultiplayer() || (GameManager.Instance.IsMultiplayer() && NetworkManager.Instance.IsMaster()))
                {
                    ...
                    if (GameManager.Instance.GameStatus != Enums.GameStatus.LoadGame)
                    {
                        ...
                        {
                            SandboxManager.Instance.DisableSandbox();
                            SandboxManager.Instance.ClearSandbox();
                        }
                        else
                        {
                            sandboxMods = (GameManager.Instance.IsObeliskChallenge() ? SaveManager.Instance.LoadPrefsString("sandboxSettingsObelisk") : SaveManager.Instance.LoadPrefsString("sandboxSettings"));
                            SandboxManager.Instance.SetSandboxMods(sandboxMods);
                        }
                        SandboxManager.Instance.LoadValuesFromCache();
                        SandboxManager.Instance.AdjustTotalHeroesBoxToCoop();
                        SandboxManager.Instance.CacheValues();
                        sandboxMods = SandboxManager.Instance.GetSandboxMods();
                    }
                    else
                    {
                        sandboxMods = SandboxManager.Instance.GetSandboxMods();
            ...
            for (int n = 0; n < 4; n++)
            {
                Hero hero2 = AtOManager.Instance.team.GetHero(n);
        ...
        public static void NodeScorePrefix()
        {
            ...
            {
                if (Globals.Instance.GetNodeData(medsMapVisitedNodes[index]) != null && Globals.Instance.GetNodeData(medsMapVisitedNodes[index]).NodeZone != null && !Globals.Instance.GetNodeData(medsMapVisitedNodes[index]).NodeZone.DisableExperienceOn__instanceZone)
                    ++num1;
            }
        ...
        public static bool SelectTomeCardsPrefix(ref TomeManager __instance, int index = -1, bool absolute = false)
        {
            ...
            {
                CardRealtimeData cardData = Globals.Instance.GetCardData(stringList[index1], false);
                    ...
                    if (medsSearchTerm.Trim() != "")
                    {
                        if (index != 22 || cardData.CardClass != Enums.CardClass.Monster)
                        {
                            ...
                            if (cardData != null && index != 22)
                            {
                                ...
                                if (cardData.UpgradesToRare != null && Globals.Instance.IsInSearch(medsSearchTerm, cardData.UpgradesToRare.Id))
        ...
        public static bool GetTeamNPCRewardPrefix(ref TierRewardData __result, ref AtOManager __instance)
        {
            ...
            for (int index = 0; index < teamNPCAtO.Length; ++index)
            {
                if (teamNPCAtO[index] != null && teamNPCAtO[index] != "")
                {
                    ...
                    if (npcData != null)
                    {
                        if (npcData != null && __instance.PlayerHasRequirement(Globals.Instance.GetRequirementData("_tier2")) && npcData.UpgradedMob != null)
                            npcData = npcData.UpgradedMob;
                        if (npcData != null && __instance.GetNgPlus() > 0 && npcData.NgPlusMob != null)
                            npcData = npcData.NgPlusMob;
                        if (npcData != null && MadnessManager.Instance.IsMadnessTraitActive("despair") && npcData.HellModeMob != null)
        ...
        public static void SetParalyzePrefix(ref CharacterItem __instance, bool state)
        {
            ...
            if (medsAnimatedSprites != null && medsAnimatedSprites.Count > 0)
            {
                ...
                if (state && medsAnimatedSprites[0].sharedMaterial == __instance.paralyzeMaterial || !state && medsAnimatedSprites[0].sharedMaterial == medsAnimatedSpritesDefaultMaterial[medsAnimatedSprites[0].name])
                    return;
                ...
                for (int index = 0; index < medsAnimatedSprites.Count; ++index)
                {
                    ...
                    if (state)
                    {
                        if ((bool)medsAnimatedSprites[index].transform.GetComponent("StealthHide"))
                        {
                    ...
                    else if ((bool)medsAnimatedSprites[index].transform.GetComponent("StealthHide"))
                    {
                        if (!medsAnimatedSprites[index].gameObject.activeSelf)
            ...
            if (state || !(medsShadowSprite != null) || medsShadowSprite.gameObject.activeSelf)
                return;
        ...
        */
        ...
        public static void SetGameDataPrefix(GameData _gameData, ref MenuSaveButton __instance)
        {
            ...
            if (_gameData.GameType == Enums.GameType.Adventure && nD != null && nD.NodeZone != null)
            {
        ...
        public static bool GetRandomCardIdByTypeAndRandomRarityPrefix(ref string __result, Enums.CardType _cardType)
        {
            ...
            int randInt = MatchManager.Instance.Random.GetRandomIntRange(0, Globals.Instance.CardListByType[_cardType].Count);
            ...
            if (_cardType == Enums.CardType.Food)
            {
                ...
                randInt = MatchManager.Instance.Random.GetRandomIntRange(0, 100);
                if (randInt > 75)
                {
            ...
            cardData = Globals.Instance.GetCardData(cardId, false);
            // CardRealtimeData cardData = Globals.Instance.GetCardData(Globals.Instance.CardListByType[_cardType][MatchManager.Instance.GetRandomIntRange(0, Globals.Instance.CardListByType[_cardType].Count)], false);
            if (cardData == null)
            {
                LogDebug($"GetRandomCardIdByTypeAndRandomRarityPrefix - CardRealtimeData is null for cardId: {cardId}");
                // __result = "";
                return false;
            }
            __result = Functions.GetCardByRarity(MatchManager.Instance.Random.GetRandomIntRange(0, 100), cardData);
            return false;
        }
